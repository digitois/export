...
drop policy if exists company_profiles_select_org on public.company_profiles;
create policy company_profiles_select_org on public.company_profiles
  for select using (public.is_org_member(organization_id));
drop policy if exists company_profiles_insert_org on public.company_profiles;
create policy company_profiles_insert_org on public.company_profiles
  for insert with check (public.is_org_member(organization_id));
drop policy if exists company_profiles_update_org on public.company_profiles;
