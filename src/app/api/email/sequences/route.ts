  ...
  try {
    ...
    switch (action) {
      ...
      case 'enrollment-action': {
        ...
        if (!handler) return apiError('Unknown enrollment op', 400);
        const result = await handler(ctx.supabase, ctx.organizationId, body.enrollmentId);
        if (result.error) return apiError(result.error.message, 400);
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    if (!id) return apiError('Sequence id required', 400);
